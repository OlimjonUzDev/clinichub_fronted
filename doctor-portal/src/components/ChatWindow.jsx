...
import { LoadingState, EmptyState, ErrorState } from './ui/StateMessage';
...
export default function ChatWindow({ appointmentId }) {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      setSendError(t('chat.send_error'));
    } finally {
      setSending(false);
    }
  };
    ...
    <div className="bg-white border border-gray-200 rounded-xl flex flex-col min-h-[400px] max-h-[70vh]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {loading ? (
          ...
          <ErrorState text={t('chat.load_error')} />
        ) : messages.length === 0 ? (
          <EmptyState icon={MessageCircle} text={t('chat.no_messages')} bordered={false} className="h-full" />
        ) : (
          messages.map((m) => {
      ...
      {sendError && (
        <div className="px-3 pt-2">
          <ErrorState text={sendError} compact />
        </div>
      )}
